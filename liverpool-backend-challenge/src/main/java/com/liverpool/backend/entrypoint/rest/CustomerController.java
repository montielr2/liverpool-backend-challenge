...
public class CustomerController {
    ...
    @Operation(summary = "Crear cliente", description = "Crea un nuevo cliente y lo persiste en la base de datos")
    @PostMapping
    public ResponseEntity<CustomerResponse> create(@Valid @RequestBody CreateCustomerRequest request) {
        return ResponseEntity.status(201).body(createCustomerUseCase.execute(request));
    }

    @Operation(summary = "Consultar cliente por userId", description = "Obtiene la información de un cliente a partir de su identificador de usuario")
    @GetMapping("/{userId}")
