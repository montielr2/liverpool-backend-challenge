...
public class SyncCustomerOrdersUseCase {
    ...
    public SyncCustomerOrdersUseCase(CustomerRepositoryPort customerRepositoryPort, OrdersProviderPort ordersProviderPort) {
    ...
    public CustomerResponse execute(String userId) {
        ...
        List<CustomerOrder> orders = ordersProviderPort.findOrderRefsByUserId(userId).stream().map(CustomerOrder::new).toList();
